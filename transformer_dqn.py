...
class QNetwork(nn.Module):
    def __init__(self,
        ...
        self.claim_fc = nn.Linear(3*hidden_size,hidden_size)
    ...
    def forward(self, claims, evidences, evidences_mask): # batch = 1
        '''
        ...
        # dim of claims reduce to hidden_dim
        claims = self.claim_fc(claims)
        output = self.clm_evi(
...
class TransformerDQN(BaseDQN):
    ...
    def convert_to_inputs_for_select_action(self, batch_state: List[State], batch_actions: List[List[Action]]) -> List[
        dict]:
        assert len(batch_state) == len(batch_actions)
        batch_claims, batch_evidences = [], [] # batch_evidences=[tensor([tensor,],)*,]
        # 状态的claim嵌入以及evi+act嵌入
        for state, actions in zip(batch_state, batch_actions):
            # tokens here is actually the sentence embedding
            ## [1, dim]
            batch_claims.extend([state.claim.tokens] * len(actions)) # [[tensor,]* ,]
            ## [seq, dim]
            evidence = [sent.tokens for sent in state.candidate] # [[tensor,],]

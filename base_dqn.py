...
from structure import Transition, Action, State
...
class BaseDQN:
    ...
    def update(self, transitions: List[Transition], isweights: List[float] = None) -> float:
        ...
        if not all(batch.done):
            # 从memory提取的数据，当done未完成，提取next_state,next_actions
        ...
        state_action_values = scores.view(-1)
        del scores
    ...
    def select_action(self,
        ...
        q_values = None
        with torch.no_grad():
            batch_inputs = self.convert_to_inputs_for_select_action(batch_state, batch_actions) # dict
            K, *DIM = list(batch_inputs.values())[0].size() # batch_size * actions ; 768
            q_values = [net(
                **dict(map(lambda x: (x[0], x[1][i].to(self.device)),
                           batch_inputs.items()))
            )[0] for i in range(0, K)]
            q_values = torch.cat(q_values, dim=0)

        batch_selected_action, offset = [], 0
        for state, actions in zip(batch_state, batch_actions):
            cur_q_values = q_values[offset:offset + len(actions)]  # 获得这个state所有动作的分值
            if is_greedy or self.epsilon_greedy:
                sent_id = cur_q_values.argmax().item()  # 选择q值最大的动作
            else:
            ...
            offset += len(actions)
        assert offset == q_values.size(0)  # batch_size * actions
